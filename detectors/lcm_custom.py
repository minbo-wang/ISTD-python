...
class LCM_Custom(BaseDetector):
    def __init__(self):
        super(LCM_Custom, self).__init__()
    
    def process(self, I_in):
        # 转为double型
        orig_shape = I_in.shape
        I_in = cv2.resize(I_in, (256, 256), interpolation=cv2.INTER_LINEAR)

        I_in = np.double(I_in)
        # 计算最终的Multiscale LCM
        [C_hat, max_margin] = self.MLCM_computation(I_in)

        # 计算均值、标准差
        mean_C_hat = np.mean(C_hat)
        sqrt_C_hat = np.sqrt(self.sqrt_matrix(C_hat, mean_C_hat))

        # 计算阈值
        k_Th = 4
        threshold = mean_C_hat + k_Th * sqrt_C_hat
        # 根据阈值判断，输出二值探测结果和统计小目标在mask中占据的像元数
        [I_out, target_pixel_num] = self.target_detection(C_hat, threshold, max_margin, I_in)
        I_out = cv2.resize(I_out, (orig_shape[1], orig_shape[0]), interpolation=cv2.INTER_LINEAR)
        self._result['target'] = I_out
        return I_out

    @classmethod
    def LCM_computation(cls, patch_LCM_in):
    ...
    @classmethod
    def MLCM_computation(cls, I_MLCM_in):
        ...
        for i in range(l_max):  # 对应不同尺度
            for j in range(0, row - scales[i] + 1):  # 单一尺度下以patch为单位做遍历，j是行
                for k in range(0, col - scales[i] + 1):  # k是列
                    ...
                    C_n = cls.LCM_computation(temp_patch)  # 对patch执行Algorithm 1
    ...
    @classmethod
    def target_detection(cls, C_hat, threshold, max_margin, I_in):
        # 用阈值生成mask
        row, col = C_hat.shape
    ...
    @classmethod
    def sqrt_matrix(cls, C_hat, mean_C_hat):
